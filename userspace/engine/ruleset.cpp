...
void falco_ruleset::ruleset_filters::add_filter(std::shared_ptr<filter_wrapper> wrap)
{
	...
	if(fevttypes.empty() || fevttypes.size() > 100)
	{
		// Should run for all event types
		add_wrapper_to_list(m_filter_all_event_types, wrap);

		// Print a warning
		fprintf(stderr, "Rule %s: warning (no-evttype):\n", wrap->name.c_str());
		fprintf(stderr, "         did not contain any evt.type restriction, or has a too lenient evt.type restriction, meaning it will run for all/almost all event types.\n");
		fprintf(stderr, "         This has a significant performance penalty. Consider adding an evt.type restriction or restructuring the rule to be more specific about event types, if possible.\n");
	}
	else
	{
